...
app.get("/", function (req, res) {
  ...
  if (!user) return res.sendFile(__dirname + "/GUI/main.html");

  let player = Player.get(user.id);
  if (
    !player ||
    board.matchEnded(player.matchId) ||
    board.isMatchDraw(player.matchId)
  ) {
    delete req.session.user;

    if (player)
      // if match ended and player existed
...
app.post("/start", function (req, res) {
  ...
  let new_player = Player.new({
    name: req.body.name,
    opposition: "",
    avatar_id: Math.floor(Math.random() * 15) + 1,
    started: false,
    symbol: "",
    lock: -1,
    matchId: null,
  ...
  if (new_player.started == true) {
    ...
    let d = {
      ...
      opponent_sym: cur_player.symbol,
      avatar_id: cur_player.avatar_id,
    };
...
app.put("/move", (req, res) => {
  ...
  if (winner) io.emit("match_ended", JSON.stringify(winner));

  let is_draw = board.matchDraw(player.matchId);
  if (is_draw) io.emit("match_draw", JSON.stringify({ msg: "Match draw!" }));
...
app.get("/end", (req, res) => {
  ...
  if (board.hasMatch(player.matchId)) board.endMatch(player.matchId);
...
http.listen(PORT ,"192.168.31.158",  function () {
